...
static void sum_of_products(fixed_t *input_mat, fixed_t *coupling_terms, fixed_t *output_mat)
{
	...
	for (uint32_t sum_i = 0; sum_i < DIGIT_CAPS_NUM_DIGITS; ++sum_i)
	{
		...
		{
            #pragma HLS PIPELINE II=1
			fixed_t partial_sums[DIGIT_CAPS_INPUT_CAPSULES] = {0.0};  // Accumulate the sum

            #pragma HLS ARRAY_PARTITION variable=partial_sums dim=1 type=complete
			...
			for (uint32_t sum_k = 0; sum_k < DIGIT_CAPS_INPUT_CAPSULES; ++sum_k)
			{
				...
				partial_sums[sum_k] += input_mat[lin_index] * coupling_terms[sum_i * DIGIT_CAPS_INPUT_CAPSULES + sum_k];
			}
            fixed_t sum;
