...
typedef struct {
    ...
    XRelu_conv_2d_uio_map maps[ MAX_UIO_MAPS ];
} XRelu_conv_2d_uio_info;

/***************** Variable Definitions **************************************/
static XRelu_conv_2d_uio_info uio_info;
...
static int uio_info_read_name(XRelu_conv_2d_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XRelu_conv_2d_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XRelu_conv_2d_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XRelu_conv_2d_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XRelu_conv_2d_Initialize(XRelu_conv_2d *InstancePtr, const char* InstanceName) {
	XRelu_conv_2d_uio_info *InfoPtr = &uio_info;
