...
void run(float *input, float *weights, float *output) {
	...
	float inputC[INPUT_NUM_CAPSULE * INPUT_DIM_CAPSULE];
	...
	float outputC[NUM_CAPSULE * DIM_CAPSULE];
    ...
    int numIter = 3;
    dynamicRoutingLoop: for (int i = 0; i < numIter; i++) {
        getSoftmax(b, c);
...
void getAdd(float *A, float *B) {
    addLoop: for (uint32_t i = 0; i < NUM_CAPSULE * INPUT_NUM_CAPSULE; i++) {
...
void getAgreement(float *inputHat, float *outputsSquash, float *agreement) {
    ...
    agreementOuterLoop: for (int i = 0; i < NUM_CAPSULE; i++) {
        agreementInnerLoop: for (int j = 0; j < INPUT_NUM_CAPSULE; j++) {
...
void getInputHat(float *input, float *weights, float *output) {
	...
	inputHatOuterLoop: for (uint32_t i = 0; i < NUM_CAPSULE; i++) {
		inputHatInnerLoop: for (uint32_t j = 0; j < INPUT_NUM_CAPSULE; j++) {
...
void matmulInputHat(float *matA, float *matB, float *matC, uint32_t startIndexA, uint32_t startIndexB, uint32_t startIndexC) {
	...
	matmulOuterLoopInputHat: for (uint32_t i = 0; i < DIM_CAPSULE; i+=2) {
			...
			matmulInnerLoopInputHatA: for (uint32_t m = 0; m < INPUT_DIM_CAPSULE; ++m) {
			...
			matmulInnerLoopInputHatB: for (uint32_t k = 0; k < INPUT_DIM_CAPSULE; ++k) {
...
void matmulAgreement(float *matA, float *matB, float *matC, uint32_t startIndexA, uint32_t startIndexB, uint32_t startIndexC) {
	// dot product between rows of matA and cols of matB
	float sum = 0.0;
	matmulInnerLoopAgreement: for (uint32_t k = 0; k < DIM_CAPSULE; ++k) { // colA = 8/16
		#pragma HLS PIPELINE
...
void getSoftmax(float *matA, float *matB) {
	...
	softmaxOuterLoop: for (uint32_t j = 0; j < INPUT_NUM_CAPSULE; j++) {
		float sum = 0.0;
        ...
        // calculate softmax
		softmaxFindSumLoop: for (uint32_t k = 0; k < NUM_CAPSULE; k++) {
			#pragma HLS PIPELINE
		...
		softmaxDivideLoop: for (uint32_t k = 0; k < NUM_CAPSULE; k++) {
			#pragma HLS PIPELINE
...
float getMaxColumn(float *mat, uint32_t starting_index) {
    ...
    maxLoop: for (uint32_t i = 0; i < NUM_CAPSULE; i++) {
		#pragma HLS PIPELINE
...
void multiply(float *matA, float *matB, float *matC, uint32_t k, uint32_t j, float mult_value) {
	...
	multiplyInnerLoop: for (uint32_t l = 0; l < DIM_CAPSULE; l++) {
		#pragma HLS PIPELINE
...
void getMultiply(float *matA, float *matB, float *matC) {
	...
	multiplyOuterLoop: for (uint32_t j = 0; j < NUM_CAPSULE; j++) {
		multiplyMidLoop: for (uint32_t k = 0; k < INPUT_NUM_CAPSULE; k++) {
...
void getSum(float *matA, float *matB) {
    ...
    sumOuterLoop: for (uint32_t j = 0; j < NUM_CAPSULE; j++) {
    	// find start value for per capsule
    	uint32_t indexSize = j * INPUT_NUM_CAPSULE * DIM_CAPSULE;
        sumMidLoop: for (uint32_t k = 0; k < DIM_CAPSULE; k+=2) {
        	...
        	// sum along columns
            sumInnerLoop: for (uint32_t l = 0; l < INPUT_NUM_CAPSULE; l++) {
...
void getSquash(float *matA, float *matB) {
    ...
    squashOuterLoop: for (int i = 0; i < NUM_CAPSULE; i++) {
    	float multVal = matB[i];
    	uint32_t indexSize = i * DIM_CAPSULE;
        squashInnerLoop: for (int j = 0; j < DIM_CAPSULE; j+=2) {
...
void getSquaredNorm(float *matA, float *matB) {
    ...
    squaredNormOuterLoop: for (int i = 0; i < NUM_CAPSULE; i++) {
		#pragma HLS PIPELINE
    	float val = 0.0;
    	float sum = matB[i];
        squaredNormInnerLoop: for (int j = 0; j < DIM_CAPSULE; j++) {
...
void getScale(float *matB) {
    scaleLoop: for (uint32_t i = 0; i < NUM_CAPSULE; i++) {
		#pragma HLS PIPELINE
...
void initialiseInputAndWeightArray() {
    for (uint32_t i = 0; i < g_weightSize; i++) { weights[i] = i % 100; }
    for (uint32_t i = 0; i < g_inputSize; i++) { input[i] = i % 100; }
}
